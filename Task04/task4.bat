...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "WITH RECURSIVE birthdays(year, date, day_of_week) AS (SELECT 2004, date('2004-04-13'), strftime('%%w', '2004-04-13') UNION ALL SELECT year + 1, date((year + 1) || '-04-13'), strftime('%%w', date((year + 1) || '-04-13')) FROM birthdays WHERE year < 2025) SELECT year, date, CASE day_of_week WHEN '0' THEN 'Воскресенье' WHEN '1' THEN 'Понедельник' WHEN '2' THEN 'Вторник' WHEN '3' THEN 'Среда' WHEN '4' THEN 'Четверг' WHEN '5' THEN 'Пятница' WHEN '6' THEN 'Суббота' END AS day_name FROM birthdays"
echo " "
